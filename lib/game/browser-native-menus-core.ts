...
export async function createBrowserNativeMenus(options:BrowserNativeMenuOptions){
 ...
 const globalReplayChoices=async():Promise<ModernReplayChoice[]>=>{
  const choices:ModernReplayChoice[]=files.enumerate('','.rpl').map(filename=>({
   label:'STUNTS / '+filename,path:'',name:filename.replace(/\.rpl$/i,''),
  }));
  if(tauriCore){
   try{
    const custom=await tauriCore.invoke<string[]>('list_custom_replays');
    for(const value of custom){
     const relative=value.replace(/\\/g,'/'),filename=relative.slice(relative.lastIndexOf('/')+1);
     choices.push({label:'Custom Tracks / '+relative,path:'',name:filename.replace(/\.rpl$/i,''),customPath:relative});
    }
   }catch(reason){console.warn('[Replay Browser] Could not scan Custom Tracks:',reason);}
  }
 ...
 return {configuration,track,elapsedSinceInputPoll:input.elapsedSinceInputPoll,selectOptions,selectCar:car,selectOpponent:opponent,selectTrack,setInputActive:input.setActive,settings:drivingSettings,get replay(){return replay;},get selectedReplay(){return selectedReplay;},get replayLoading(){return replayLoading;},finishReplayLoading(){replayLoading?.close();replayLoading=undefined;},consumeRaceSpawn(){const spawn=pendingRaceSpawn;pendingRaceSpawn=undefined;return spawn;},reopenTrackEditor:editTrack,raceEntryKey:fastForwardKey,fadeMusic:()=>music.fadeOut(input.waitTicks),close:()=>{replayLoading?.close();replayLoading=undefined;window.removeEventListener(ENHANCED_TEXTURES_EVENT,syncEnhancedTextures);window.removeEventListener(ENHANCED_FOV_EVENT,syncEnhancedFov);input.close();files.close();},
  ...
  showRaceWaiting(memory:Uint8Array){show('race');canvas.style.cursor='none';if(options.graphics?.enabled)return;if(lastNativeDisplay){const {owner}=lastNativeDisplay,high={cga:0x5e0,tandy:0x620,ega:0x45c}[owner.mode],y=new DataView(memory.buffer,memory.byteOffset,memory.byteLength).getUint16(0x2d1a0+0x8a10,true);new DataView(owner.memory().buffer).setUint16(owner.d+0x8a10+high,y,true);restoreOriginalDisplayWindow(owner.memory(),owner.d,owner.mode);drawOriginalRaceWaitingDisplay(owner.memory(),owner.d,owner.mode,owner.drawing,host.resources.ewai,0xe800);pixels.set(lastNativeDisplay.pixels());paint(lastNativeDisplay.palette);return;}drawOriginalRaceWaiting(pixels,font,host.resources.ewai,memory,0x2d1a0);present();},
