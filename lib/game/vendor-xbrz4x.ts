...
export function xbrz4xCanvas(source:HTMLCanvasElement):HTMLCanvasElement{
 ...
 if(!context||width<=0||height<=0)return source;
 const rgba=context.getImageData(0,0,width,height).data;
 const src=new Array<number>(width*height);
 for(let i=0;i<src.length;i++){
  const at=i*4,r=rgba[at],g=rgba[at+1],b=rgba[at+2],a=rgba[at+3];
  src[i]=((a<<24)|(r<<16)|(g<<8)|b)>>>0;
 }
 const out=new Array<number>(width*height*16).fill(0);
 scaleImage(4,src,out,width,height,0,height);
 const canvas=document.createElement('canvas');
 canvas.width=width*4;canvas.height=height*4;
 const outContext=canvas.getContext('2d');
 if(!outContext)return source;
 const image=outContext.createImageData(canvas.width,canvas.height);
 for(let i=0;i<out.length;i++){
  const value=out[i]>>>0,at=i*4;
  image.data[at]=(value>>>16)&255;
  image.data[at+1]=(value>>>8)&255;
  image.data[at+2]=value&255;
  image.data[at+3]=(value>>>24)&255;
 }
