 ...
 * count+1 cumulative u32 payload offsets, then resource data.
 */
export function parseVce(bytes:Uint8Array):VceFile{
 ...
 const namesAt=6,offsetsAt=namesAt+count*4,payloadAt=offsetsAt+(count+1)*4;
 ...
 // VCE stores count+1 cumulative payload offsets. The first is normally 0
 // and the last is the exact payload length, giving an explicit end for every resource.
 const offsets=Array.from({length:count+1},(_,index)=>view.getUint32(offsetsAt+index*4,true));
 ...
 const resources=names.map((name,index)=>{
  const start=payloadAt+offsets[index],end=payloadAt+offsets[index+1];
