...
export function createBrowserMenuInput(element:HTMLCanvasElement,options:{joystickEnabled?:()=>boolean;drivingBindings?:()=>ArrayLike<number>;onPoll?:()=>void|Promise<void>}={}){
 ...
 const clear=()=>{blockedButtons|=buttons;controlHeld=false;buttons=0;pendingKey=0;held.clear();mappedHeld.clear();pointerEdges.length=0;releaseCapture();};
 ...
 const readImmediate=(deltaOverride?:number|(()=>number))=>{
  const now=counter(),delta=deltaOverride===undefined?(now-last)&65535:(typeof deltaOverride==='function'?deltaOverride():deltaOverride)&65535;last=now;lastPoll=now;const pad=gamepad(),pointer=pointerEdges.shift()??{x,y,buttons:active?buttons:0},keyboardKey=takeMappedKey();
 ...
 return {
  ...
  async keyboard(){await wait();const ticks=counter();return {key:takeMappedKey(),input:ticks>>>0,game:Math.floor(ticks/ORIGINAL_GAME_TIMER_DIVIDER)>>>0};},
