...
export function createModernCarShowroom(palette:number[],indices:number[]){
 ...
 // Match app/Garage.tsx exactly. The previous mesh-grid workaround only
 // compensated for the old double-composited preview path.
 ...
 return {
  ...
  close(){
   ...
   renderer.domElement.remove();renderer.dispose();renderer.forceContextLoss();
  }
 };
}
