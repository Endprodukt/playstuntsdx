/** Broderbund Stunts 1.1 update_grip.
 * Source of truth: Restunts BB1.1 asmorig/seg001.asm update_grip.
 * Deliberately independent from the Mindscape Dec-1990 implementation.
 */
...
export function stepGrip(before:GripState,t:GripTuning,player=true,trackElement=0,onContactScratch?:(words:[number,number])=>void):GripState {
 ...
 // BB1.1 performs signed long multiplication followed by ten arithmetic shifts.
 const combined=i16(Math.imul(i16(t.grip*2),sum)>>10);
 ...
 if(player){
  ...
  if(demand>combined){
   ...
   // BB1.1 181AD: AX = 3*angle + initial, then SAR AX twice.
   angle=i16(i16(i16(angle*3)+initial)>>2);
   s.slip=i16(initial-angle);
  }else{
   s.sliding=0;
   if(s.slip){s.slip=i16(s.slip-(i16(s.slip)>>4));if(Math.abs(s.slip)<16)s.slip=i16(s.slip)>>1}
  }
  ...
  if(s.spin)s.spin=i16(i16(s.spin*15)>>4);
 }
 if(s.wheelAngle && !s.spin)s.wheelAngle=i16(i16(s.wheelAngle*15)>>4);
