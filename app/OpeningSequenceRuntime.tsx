  ...
  async function run(){
   ...
   for(;;){
    ...
    if(transition.type==='drive'||transition.type==='replay'){
     ...
     setTrackExplanation('');setStatus(recording?'Preparing replay':'Preparing race');replayLoading?.update('Preparing replay assets','Loading shared Stunts runtime data');await menus.fadeMusic();replayLoading?.throwIfCancelled();if(disposed)return;
