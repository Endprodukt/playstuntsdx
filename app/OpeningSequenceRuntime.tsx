...
import {ENHANCED_RENDER_SCALE_EVENT,enhancedRenderResolution,enhancedRenderScale} from '@/lib/game/enhanced-resolution-settings';
...
export default function OpeningSequence({assets,onBack,backLabel="← Back",soundDevice,displayMode,initiallyMuted=false,hercules=false,directory="C:\\",initialTrack,onRolandDevice,onRolandPower,rolandPower,onRunningChange,embedded=false,autoStart=false}:{autoStart?:boolean;embedded?:boolean;assets:Assets;onBack:()=>void;backLabel?:string;soundDevice?:'pc-speaker'|'mt32'|'tandy';initiallyMuted?:boolean;hercules?:boolean;displayMode?:NativeBrowserDisplayMode;directory?:string;initialTrack?:number[];onRunningChange?:(running:boolean)=>void;rolandPower?:BrowserMt32Power;onRolandDevice?:(device:BrowserNativeMt32Device|undefined)=>void;onRolandPower?:(power:Awaited<ReturnType<typeof createBrowserNativeMt32Music>>['power']|undefined)=>void}){
 ...
 const canvas=useRef<HTMLCanvasElement>(null),[renderScale,setRenderScale]=useState(enhancedRenderScale),[status,setStatus]=useState('Ready to play Stunts'),[error,setError]=useState('');
 const [trackExplanation,setTrackExplanation]=useState('');
 useEffect(()=>{const changed=()=>setRenderScale(enhancedRenderScale());window.addEventListener(ENHANCED_RENDER_SCALE_EVENT,changed);return()=>window.removeEventListener(ENHANCED_RENDER_SCALE_EVENT,changed);},[]);
 useEffect(()=>{graphics.current.refresh?.();},[renderScale,upgraded]);
 ...
 useEffect(()=>{
  ...
  async function run(){
   ...
   if(!skipped){
    ...
    const displayAnimation=()=>{if(graphics.current.enabled&&lastDraw&&lastPose){introGpu??=createUpgradedIntro(renderer.memory,introMaterials);const internal=enhancedRenderResolution();upgradedContext.setTransform(1,0,0,1,0,0);upgradedContext.drawImage(introGpu.draw(lastDraw,lastPose,internal.width,internal.height),0,0,upgradedSurface.width,upgradedSurface.height);}context.setTransform(1,0,0,1,0,0);context.imageSmoothingEnabled=false;context.drawImage(graphics.current.enabled?upgradedSurface:originalSurface,0,0,element.width,element.height);};
   ...
   for(;;){
    ...
    if(transition.type==='drive'||transition.type==='replay'){
     ...
     const replayLoading=transition.type==='replay'?menus.replayLoading:undefined;
     ...
     try{
      setTrackExplanation('');setStatus(recording?'Preparing replay':'Preparing race');replayLoading?.update('Preparing replay assets','Loading shared Stunts runtime data');
      await menus.fadeMusic();replayLoading?.throwIfCancelled();if(disposed)return;
      if(recording)replayLoading?.update('Preparing replay assets','Loading physics, track and resource catalogs');
      demoData??=await loadBrowserNativeDemoData(assets);replayLoading?.throwIfCancelled();if(soundDevice==='pc-speaker')demoData.soundDevice={kind:'pc-speaker',port61:()=>0};else if(soundDevice==='tandy')demoData.soundDevice={kind:'tandy',port61:()=>0,interruptCx:()=>0,bios:serviceSuppliedTandySilentRequests};else if(soundDevice==='mt32')demoData.soundDevice={kind:'mt32',execute:executeReadyMt32Program,async executeInitialization(program){const writes:number[][]=[];const result=await executeCooperativeReadyMt32Program(program,{write(batch){writes.push(...batch);},cancelled:()=>disposed});return {result,writes};}};if(disposed)return;
      restored=await runBrowserNativeManualRace({context:runAudio,mt32Output:roland?.output,displayMode,hercules,data:demoData,menus,replay:recording,replayLoading,spawn:editorSpawn,editorTest,menu:{configuration:transition.configuration,track:menus.track.raw,name:menus.track.name,path:menus.track.path,camera:demoCamera,randomState:demoRandomState,retainedSession:takeRetainedSession(),graphics:menus.settings.graphics,mouse:menus.settings.mouse,joystick:menus.settings.joystick,soundEnabled:music.settings.soundEnabled},signal:demoAbort.signal,stopMusic:music.stop,onStage(stage){if(disposed)return;if(stage==='race'){document.getElementById('playstunts-editor-test-loading')?.remove();if(recording){replayLoading?.update('Replay ready','Playback started');menus.finishReplayLoading();}}performanceRunning.current=stage==='race';if(performanceRunning.current)graphics.current.setPerformancePaused?.(false);else{graphics.current.chaseCamera=0;graphics.current.resetPerformance?.();}setStatus(({loading:'Preparing race',race:'Stunts',results:'Race results',seeking:'Preparing replay'})[stage]);},onFrame(frame,mode,clock,blocked){element.dataset.raceFrame=String(frame);element.dataset.raceMode=String(mode);element.dataset.raceClock=String(clock);element.dataset.raceClockBlocked=String(blocked);element.dataset.raceAudioState=runAudio.state; element.dataset.raceAudioTime=String(runAudio.currentTime);}});}
     catch(reason){
      document.getElementById('playstunts-editor-test-loading')?.remove();
      if(recording&&replayLoading){
       performanceRunning.current=false;graphics.current.resetPerformance?.();
       if(!replayLoading.cancelled)await replayLoading.fail(reason);
       menus.finishReplayLoading();music.play('slct');menus.setInputActive(true);
       try{
        const result=await menus.selectOptions();
        if(result==='exit'){onBack();return;}
        if(result==='replay')queuedTransition={type:'replay',configuration:menus.configuration.slice(0,24)};
       }finally{menus.setInputActive(false);}
       setStatus(queuedTransition?.type==='replay'?'Preparing replay':'Original options menu');continue;
      }
